Page({
	...
	cellClick(e) {
		...
		this.setData({dotImage: this.data.listData[index].images});
		if(this.data.dotList.length >= 10) {
			this.setData({
				dotList: []
			});
		}
		wx.createSelectorQuery().select(`#image${index}`).boundingClientRect((res) => {
			...
			dotList[index].top = destination.top;
			dotList[index].left = destination.left;
			dotList[index].scale = 0.5;
			this.setData({
				dotList: dotList
			});
			setTimeout(() => {
	...
	onLoad() {
		wx.createSelectorQuery().select(".cart").boundingClientRect((res) => {
